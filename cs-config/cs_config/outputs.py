...
def liability_plot(df_base, df_reform, span, mtr_opt):
    ...
    base_toggle = Toggle(label="Previous Law (Solid)", button_type="default",
                         active=True)
    ...
    reform_toggle = Toggle(label="American Rescue Plan (Dashed)", button_type="default",
                           active=True)
...
def credit_plot(df_base, df_reform, span, mtr_opt):
    ...
    fig = figure(plot_width=600, plot_height=500, x_range=(
        -2500, 150000), tools=tools, active_drag="pan")
    ...
    rebate_base = fig.line(x="Axis", y="Recovery rebate", line_color='#800080', muted_color='#800080',
                     line_width=2, legend_label='Recovery Rebate Credit (Stimulus Checks)', muted_alpha=0.1, source=df_base)
